...
Camera → Robot 변환이 $T_{robot,camera}$라면:
...
$T_{world,camera}=T_{world,robot}T_{robot,camera}$

Camera의 점을 World로:

$p_{world}=T_{world,robot}T_{robot,camera}p_{camera}$
...
$T_{camera,robot}=T_{robot,camera}^{-1}$
...
> 수치적으로는 직접 $(A^T A)^{-1}A^Tb$를 계산하기보다 `np.linalg.lstsq()` 같은
...
# 18. 좌표변환 표기 읽는 법
  ...
  표현               의미
  ------------------ ----------------
  $T_{world,robot}$    Robot → World
  $T_{robot,camera}$   Camera → Robot
  $T_{robot,lidar}$    LiDAR → Robot
  $T_{camera,robot}$   Robot → Camera
  $T^{-1}$              변환 방향 반대
